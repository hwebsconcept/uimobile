...
if (isset($_GET['id']))
{
    $del= $dbh->prepare("DELETE FROM coursenotification WHERE id=:id");
    ...
    <div id="wrapper">
      ...
      <div id="page-wrapper">
		...
		<table class="table table-bordered">
			...
			$a= $dbh->prepare("SELECT * FROM coursenotification WHERE staffId=:posted AND courseID=:courseID ORDER BY id DESC");
			$a->bindParam(':posted', $_SESSION['staffId']);
            $a->bindParam(':courseID', $_GET['courseID']);
			$a->execute();
			...
			<tr>
				<td><?php echo $b['id'] ?></td>
				<td><?php echo $d['courseCode']; ?> <?php echo $d['courseName']; ?></td>
				<td><?php echo $b['subject'] ?></td>
				<td><?php echo $b['message'] ?></td>
				<td><a href="edit-coursenotification.php?id=<?php echo $b['id'] ?> "><button class="btn btn-sm btn-default">Edit</button> </a></td>
				<td><a href="view-coursenotification.php?id=<?php echo $b['id'] ?> "><button class="btn btn-sm btn-warning">Delete</button> </a></td>
